...
`scripts/migrate.py` owns the deterministic work — `tiers` (validate each finding's tier against the KB tags from `gate.py rules`; missing tiers on failing findings, unknown tiers, and tag-inconsistent tiers are rejected with the fix named), `report` (settled tiered verdict → gap report: HTML plus the machine-readable JSON beside it), `fleet` (directory of settled per-repo verdicts → fleet report: HTML + JSON with verdict, tier counts, and staleness lag per repo, drift hot-spots, waiver counts). Interface: `uv run scripts/migrate.py --help`. Reports land in `{output_folder}/hex/` unless the user points elsewhere. If script execution is unavailable — interactive only — do the same checks by hand against the schemas and mark every artifact `hand-rendered, unsettled`; headless returns blocked.
