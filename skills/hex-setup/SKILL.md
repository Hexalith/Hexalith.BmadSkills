---
name: "hex-setup"
description: Sets up the BMAD Hexalith Module in a project. Use when the user requests to 'install hex module', 'configure the hexalith module', or 'setup hexalith skills'.
---
...
0. **Detect the config layout.** If `{project-root}/_bmad/config.toml` exists (BMAD ≥ 6.10 consolidated TOML layout), do **not** run the YAML merge scripts below — they target the legacy YAML layout and would create orphaned files that can shadow the real config. Instead: collect values as described, then write the module's values into `_bmad/config.toml` (and user-setting values into `_bmad/config.user.toml`) matching the table format the other modules use in those files, and register this module's rows from `./assets/module-help.csv` into `{project-root}/_bmad/_config/bmad-help.csv` (remove any existing `hex-*` rows first, then append). Skip the legacy-cleanup script too. Everything else in this skill — tool checks, output directories, fleet manifest, confirmation — applies unchanged.
...
3. Check for per-module configuration at `{project-root}/_bmad/hex/config.yaml` and `{project-root}/_bmad/core/config.yaml`. If either file exists:
...
The hex skills call external CLIs at runtime. Check each and guide installation when missing — **never block setup on a missing tool**; every skill re-checks at runtime for the specific tool it needs.
...
**Exception:** `hex_fleet_manifest` is a *file* path, not a directory — never `mkdir` it. It is handled next.

## Fleet Manifest Scaffold

If the file configured as `hex_fleet_manifest` does not exist, offer to create a starter — never create it unasked. When gh is available and authenticated and `hex_framework_source` names a GitHub org, offer to pre-fill it with the Hexalith module repos found in that org; otherwise write a commented template. Keep it minimal: a `repos:` list where each entry is a local path or an `owner/name` GitHub slug — this is what hex-migrate's fleet audit reads.
